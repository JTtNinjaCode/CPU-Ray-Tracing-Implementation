...
public:
  ...
  aabb offset(const vec3 &offset) const { return aabb(x_.offset(offset.x()), y_.offset(offset.y()), z_.offset(offset.z())); }
...
private:
  ...
  // check if the three intervals have overlap, if so, it means the ray hit the aabb
  bool overlaps(interval int1, interval int2, interval int3, interval ray_t) const {
